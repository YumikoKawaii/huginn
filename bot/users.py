...
def load_or_setup_users(base_url: str) -> list[dict]:
    ...
    if CREDS_FILE.exists():
        try:
            ...
            if users:
                print(f"  [users] Loaded {len(users)} bot users from {CREDS_FILE}")
                return users
        except Exception:
            pass

    print(f"  [users] Registering {BOT_USER_COUNT} new bot users...")
    client = ApiClient(base_url)  # no auth needed for registration
    ...
    for _ in range(BOT_USER_COUNT):
        ...
        try:
            ...
            print(f"  [users] ! Failed to register {cred['username']}: {exc}")

    CREDS_FILE.parent.mkdir(parents=True, exist_ok=True)
    CREDS_FILE.write_text(json.dumps(users, indent=2))
    print(f"  [users] Registered {len(users)} users → {CREDS_FILE}")
    return users
